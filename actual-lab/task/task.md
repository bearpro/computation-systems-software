...
* **MLT-3 (Multi Level Transmission-3):** при единице — переход на другой уровень, при нуле — уровень не меняется. Три уровня сигнала.
* **Недостатки:** отсутствие самосинхронизации, три уровня сигнала, постоянная составляющая при длинных последовательностях нулей.

##### 1.2.2.8. Пятиуровневый код PAM-5

* **PAM-5:** пять уровней сигнала; четыре уровня кодируют два бита (00, 01, 10, 11), пятый — для избыточности (исправление ошибок).
* **Достоинство:** при той же скорости модуляции данные передаются в 2 раза быстрее по сравнению с AMI или NRZ.
* **Недостатки:** постоянная составляющая при передаче одинаковых пар бит, высокая стоимость реализации.
